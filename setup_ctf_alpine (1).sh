...
setup_nginx() {
    ...
    # Port 80 — CTF progression portal (proxies to Flask on 5000)
    # /files/ is BLOCKED from direct access — files served through portal only
    # /backup/, /internal/ still accessible as T0 honeypots
...
server {
    ...
    # ── Block direct file access — must go through the portal ─────────
    # Players cannot browse /files/ directly. The Flask portal serves
    # files only after a team has unlocked the relevant tier.
    location /files/ {
        deny all;
        return 403;
    }

    # ── T0 honeypots — deliberately exposed on port 80 ────────────────
    # These are reachable without auth as part of the T0 recon challenge.
    location /backup/ {
        root /var/www/html;
        autoindex on;
    }
    ...
    location = /.env            { root /var/www/html; }
    location = /config.php.bak  { root /var/www/html; }

    # ── Everything else → CTF portal ──────────────────────────────────
    location / {
...
deploy_challenges() {
    ...
    for web in web01 web02 web03; do
        DIR="$CTF/T0-Web/WEB-$(echo $web | tr '[:lower:]' '[:upper:]')/challenge"
        [ -d "$DIR" ] && cp -r "$DIR/." "/var/www/$web/" && log "Deployed: $web" \
...
EOF
    ...
    # ── CTF Progression Portal ────────────────────────────────────────
    info "Deploying CTF progression portal..."

    mkdir -p /var/www/ctf_portal /var/lib/ctf_portal

    # Copy portal app from repo if present, otherwise use the bundled one
    if [ -f "$CTF/Platform/ctf_portal.py" ]; then
        cp "$CTF/Platform/ctf_portal.py" /var/www/ctf_portal/app.py
        log "Portal app copied from repo"
    elif [ -f "/root/Arp_101/ctf_portal.py" ]; then
        cp "/root/Arp_101/ctf_portal.py" /var/www/ctf_portal/app.py
        log "Portal app copied from repo root"
    else
        warn "ctf_portal.py not found in repo — copy it manually to /var/www/ctf_portal/app.py"
    fi

    # Generate a persistent secret key
    PORTAL_SECRET=$(python3 -c "import secrets; print(secrets.token_hex(32))")
    echo "PORTAL_SECRET=$PORTAL_SECRET" > /var/www/ctf_portal/.env
    chmod 600 /var/www/ctf_portal/.env

    chown -R root:root /var/www/ctf_portal
    chmod -R 755 /var/www/ctf_portal
    chmod 600 /var/www/ctf_portal/.env
    chown -R root:root /var/lib/ctf_portal

    # Install dependencies
    pip3 install flask gunicorn --break-system-packages >> "$LOG" 2>&1 || true

    # OpenRC service
    cat > /etc/init.d/ctf_portal << 'SVCEOF'
#!/sbin/openrc-run
name="ctf_portal"
description="CTF Progression Portal"
command="/usr/bin/gunicorn"
command_args="--workers 2 --bind 127.0.0.1:5000 app:app"
directory="/var/www/ctf_portal"
command_background=true
pidfile="/run/ctf_portal.pid"
output_log="/var/log/ctf_portal.log"
error_log="/var/log/ctf_portal.log"
env_file="/var/www/ctf_portal/.env"
depend() { need net; }
SVCEOF
    chmod +x /etc/init.d/ctf_portal
    rc-service ctf_portal start >> "$LOG" 2>&1 \
        && log "CTF portal started on 127.0.0.1:5000" \
        || warn "CTF portal start failed — check /var/log/ctf_portal.log"
    rc-update add ctf_portal default >> "$LOG" 2>&1 || true

    log "CTF portal deployed"
}
...
restart_services() {
    ...
    rc-service ctf_portal restart >> "$LOG" 2>&1 && log "CTF portal restarted" || warn "CTF portal restart failed (check /var/log/ctf_portal.log)"
...
verify() {
    ...
    echo ""
    echo "=== Services ==="
    for svc in nginx php-fpm83 sshd ctf_portal; do
    ...
    for sock in /var/run/php83-fpm-web0{1,2,3}.sock; do
        if [ -S "$sock" ]; then
            GRP=$(stat -c '%G' "$sock" 2>/dev/null)
    ...
    echo -n "  /proc hidepid=2:    "; mount | grep -q "hidepid=2" && echo "ACTIVE" || echo "pending reboot"
    ...
    echo ""
    echo "=== Portal ==="
    echo -n "  CTF Portal running: "
    curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:5000/ 2>/dev/null | grep -q "200\|302" \
        && echo -e "${GREEN}OK${NC}" || echo -e "${RED}FAILED — check /var/log/ctf_portal.log${NC}"
    echo -n "  /files/ direct blocked: "
    curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1/files/ 2>/dev/null | grep -q "403" \
        && echo -e "${GREEN}OK (403)${NC}" || echo -e "${RED}EXPOSED — fix nginx config${NC}"
    ...
    echo -e "${CYAN}  ┌─────────────────────────────────────────────────┐${NC}"
    echo -e "${CYAN}  │  PLAYER ENTRY POINT                             │${NC}"
    echo -e "${CYAN}  │  CTF Portal:    http://$IP/                │${NC}"
    echo -e "${CYAN}  │                                                 │${NC}"
    echo -e "${CYAN}  │  CHALLENGE PORTS (linked from portal)           │${NC}"
    echo -e "${CYAN}  │  WEB-01:        http://$IP:8001/          │${NC}"
    echo -e "${CYAN}  │  WEB-02:        http://$IP:8002/          │${NC}"
    echo -e "${CYAN}  │  WEB-03:        http://$IP:8003/          │${NC}"
    echo -e "${CYAN}  │  Admin Panel:   http://$IP:8080/          │${NC}"
    echo -e "${CYAN}  └─────────────────────────────────────────────────┘${NC}"
    echo ""
    echo "  Tier unlock thresholds:"
    echo "    T1 unlocks after: 3/5 T0 solves"
    echo "    T2 unlocks after: 4/7 T1 solves"
    echo "    T3 unlocks after: 3/5 T2 solves"
    echo "    T4 unlocks after: 2/3 T3 solves"
    echo ""
